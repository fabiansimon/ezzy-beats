...
def  main():
  ...
  installer = "\installer\EzzyBeat Installer.exe"
   
  if demo:
    installer = "\installer\EzzyBeat Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\EzzyBeat manual.pdf" 
  ]
  ...
  ZIPNAME = "EzzyBeat-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "EzzyBeat-v" + FULLVERSIONSTR + "-win-demo.zip"
